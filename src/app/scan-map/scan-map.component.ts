...
export class ScanMapComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    self.dataSource = self.cyhyDataService.getDataSubject(self.cyhyDataService.dataIdentifiers.SCAN_MAP);
  ...
  topo_ready(error, us) {
    const promise = new Promise((resolve, reject) => {
      ...
      self.dataSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.SCAN_MAP, self.update_hexes);
    });
  }
  ...
  update_hexes(error, hosts) {
    ...
    if (hosts && hosts.length > 0) {
      ...
      node.select("path")
        .style("fill", function(d) { return self.color(d[0].stage) }); //TODO use best color, not first
