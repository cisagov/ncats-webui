...
export class PreferencesComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    (<HTMLInputElement>$('#censor_names')[0]).checked = this.cookies.get(CookieService.CENSOR_NAMES) == 'true';
    (<HTMLInputElement>$('#election_data')[0]).checked = this.cookies.get(CookieService.ELECTION_DASHBOARD) == 'true';
    (<HTMLInputElement>$('#cycle_pages')[0]).checked = this.cookies.get(CookieService.CYCLE_PAGES) == 'true';
    (<HTMLInputElement>$('#cycle_delay')[0]).value = this.cookies.get(CookieService.CYCLE_DELAY) || '300';
    (<HTMLInputElement>$('#cycle_delay')[0]).value = this.cookies.get(CookieService.CYCLE_DELAY) || '300';
  }
  ...
  _initializeUnderscan(): void {
    this.verticalUnderscan = this.cookies.get(CookieService.VERTICAL_UNDERSCAN) || '0';
    $('#vertical-underscan-slider').attr(
      "data-options", $('#vertical-underscan-slider').attr("data-options") + "initial: " + this.verticalUnderscan + ";"
    );

    this.horizontalUnderscan = this.cookies.get(CookieService.HORIZONTAL_UNDERSCAN) || '0';
    $('#horizontal-underscan-slider').attr(
      "data-options", $('#horizontal-underscan-slider').attr("data-options") + "initial: " + this.horizontalUnderscan + ";"
    );
  }

  private _initializeCycleList(): void {
    this.cycleList = this.cookies.getJSON(CookieService.CYCLE_LIST);
    this.componentList = this.pageCycle.getCyclableComponents()
  ...
  _getFoundationEventHandlers(): Object {
    return {
      slider: {
        on_change: () => {
          this.verticalUnderscan = $('#vertical-underscan-slider').attr('data-slider');
          this.cookies.set(CookieService.VERTICAL_UNDERSCAN, this.verticalUnderscan);
          ...
          this.horizontalUnderscan = $('#horizontal-underscan-slider').attr('data-slider');
          this.cookies.set(CookieService.HORIZONTAL_UNDERSCAN, this.horizontalUnderscan);
