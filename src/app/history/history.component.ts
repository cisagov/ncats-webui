...
export class HistoryComponent implements OnInit, OnDestroy {
  ...
  cellData(d: any): any
  {
    ...
    let age = Math.round((now.getTime() - endDate.getTime()) / (100*60*60*24)) / 10;
    let duration = Math.round((endDate.getTime() - startDate.getTime()) / (100*60*60*24)) / 10;
  ...
  redraw(jsonData: any): void
  {
    ...
    let rows = d3.select("#snapshot-table").select("tbody").selectAll("tr")
      .data(jsonData, function(d:any) { return d.owner; });
