...
export class StatusComponent implements OnInit, OnDestroy {
  ...
  constructor(private titleService: CyhyTitleService, private cyhyDataService: CyhyDataService, private router: Router) {
    self = this;
  ...
  ngOnInit() {
    ...
    setTimeout(() => { // Give time for component-specific CSS to load
      let data = self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.STATUS_ALL);
      self.update(undefined, data);
    }, 100);

    self.dataSource = self.cyhyDataService.getDataSubject(self.cyhyDataService.dataIdentifiers.STATUS_ALL);
    if (self.dataSource) self.dataSource.refreshData();
    self.dataSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.STATUS_ALL, self.update);
  ...
  bottomOfH1(): any{
      ...
      return placeholder.offsetTop + placeholder.offsetHeight + self.CHART_SPACING * 2;
  }

  calculateChartTop(d: any, i: number): any {
      return (i * (self.height + self.CHART_SPACING) + self.bottomOfH1()).toString() + 'px';
  }
  ...
  update(error: any, jsonData: any): void
  {
      ...
      if (jsonData) {
        ...
        //BIND
        var svg = d3.select("body").select("body").select(".placeholder").selectAll("svg")
            .data(bullets, function(d) { try { return d.title; } catch(e) { console.log(e); } });
