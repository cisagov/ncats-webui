...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    self.update_all_charts(undefined, self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.DASHBOARD_QUEUE));

    if(this.showElectionData){
      self.update_overall_metrics(undefined, self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.DASHBOARD_ELECTION_METRICS));
      self.update_stakeholders(undefined, self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.DASHBOARD_ELECTION_SEVERITY));
      document.getElementById("metrics").textContent = "ELECTION METRICS";
      document.getElementById("stakeholder_type").textContent = "ELECTION STAKEHOLDER";
      document.getElementById("number_of_stakeholders").textContent = "ELECTION STAKEHOLDERS";
    }
    else{
      self.update_overall_metrics(undefined, self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.DASHBOARD_METRICS));
      self.update_stakeholders(undefined, self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.DASHBOARD_SEVERITY));
      ...
      document.getElementById("number_of_stakeholders").textContent = "STAKEHOLDERS";
    }

    self.queueSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.DASHBOARD_QUEUE, self.update_all_charts);
    if(this.showElectionData){
      self.electionMetricsSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.DASHBOARD_ELECTION_METRICS, self.update_overall_metrics);
      self.electionSeverityCountsSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.DASHBOARD_ELECTION_SEVERITY, self.update_stakeholders);
    }
    else{
      self.metricsSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.DASHBOARD_METRICS, self.update_overall_metrics);
      self.severityCountsSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.DASHBOARD_SEVERITY, self.update_stakeholders);
  ...
  ngOnDestroy() {
    ...
    if (self.severityCountsSubscription) self.severityCountsSubscription.unsubscribe();
    if (self.electionMetricsSubscription) self.electionMetricsSubscription.unsubscribe();
    if (self.electionSeverityCountsSubscription) self.electionSeverityCountsSubscription.unsubscribe();
  }

  check_cookies(): void {
    self.censorNames = (this.cookies.get(CookieService.CENSOR_NAMES) === 'true');
    self.showElectionData = (this.cookies.get(CookieService.ELECTION_DASHBOARD) === 'true');
  }
  ...
  update_overall_metrics(err, data): void {
    ...
    if (data) {
      let json_data = JSON.parse(data.data);
      if (json_data) {
        self.update_single_overall_metric(json_data.stakeholders, '#stakeholders');
        self.update_single_overall_metric(json_data.addresses, '#addresses');
        self.update_single_overall_metric(json_data.hosts, '#hosts');
        self.update_single_overall_metric(json_data.vulnerable_hosts, '#vulnerable_hosts');
        self.update_single_overall_metric(json_data.open_tickets.total, '#vulns_total');
        self.update_single_overall_metric(json_data.open_tickets.critical, '#vulns_critical');
        self.update_single_overall_metric(json_data.open_tickets.high, '#vulns_high');
        self.update_single_overall_metric(json_data.open_tickets.medium, '#vulns_medium');
        self.update_single_overall_metric(json_data.open_tickets.low, '#vulns_low');
