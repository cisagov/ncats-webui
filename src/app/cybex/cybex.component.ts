...
export class CybexComponent implements OnInit, OnDestroy {
  ...
  constructor(private titleService: CyhyTitleService, private config: CyhyConfigService, private cyhyDataService: CyhyDataService, private route: ActivatedRoute) {
    ...
    enum chart {ALL, ONE, TWO, THREE, FOUR};
    self.chart = chart;
  }
  ...
  ngAfterViewInit() {
    self.create_charts();
    self.update_all_charts(undefined, self.cyhyDataService.getData(self.cyhyDataService.dataIdentifiers.CYBEX_ALL));
    self.dataSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.CYBEX_ALL, self.update_all_charts);
  }
