...
export class HostMapComponent implements OnInit {
  ...
  constructor(private titleService: CyhyTitleService, private cyhyDataService: CyhyDataService) {
    self = this;
  ...
  ngOnInit() {
    ...
    self.color_scale = d3.scale.threshold()
    ...
    self.dataSource = self.cyhyDataService.getDataSubject(self.cyhyDataService.dataIdentifiers.HOST_MAP);
  ...
  topo_ready(error: any, us: any): void {
    const promise = new Promise((resolve, reject) => {
      ...
      self.dataSubscription = self.cyhyDataService.subscribeToSubject(self.cyhyDataService.dataIdentifiers.HOST_MAP, self.update_hexes);
    });
  }
  ...
  update_hexes(error: any, hosts: any): void {
    ...
    if (hosts && hosts.length > 0) {
      ...
      var node = self.svg.selectAll(".hexagons")
          .data(self.hexbin(hosts).sort(function(a, b) { return b.length - a.length; }))
      ...
      node.select("path")
        .style("fill", function(d) { return self.color_scale(d.length) });
