...
export class StatusDetailComponent implements OnInit {
  private static readonly STAGES: string[] = ['NETSCAN1','NETSCAN2','PORTSCAN','VULNSCAN','BASESCAN'];
  private static readonly STATUS: string[] = ['WAITING','READY','RUNNING','DONE'];
  ...
  updateCustomerSubscription(acronym: string): void {
    if (this.customerDataSource) {
      this.customerDataSource.setPath([this.config.get('data_path'), 'status', acronym]);
  ...
  updateTallyTableData(tally: Object): void {
    ...
    let counts = this._orderBy(StatusDetailComponent.STAGES, tally['counts'], true);
  ...
  _setupCustomerSubscription(acronym: string): Subscription {
    this.customerDataSource = new CyhyJsonDataSource(
      ...
      'j', StatusDetailComponent.SUBSCRIPTION_REFRESH_TIME
    );
